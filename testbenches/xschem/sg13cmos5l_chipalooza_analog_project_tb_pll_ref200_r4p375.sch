...
F {}
E {}
T {PLL bench ref200_r4p375: 200 MHz reference in, PRBS-7 at 437.5 Mb/s out.

  fractional-N, code 35: three eighth bits, and the slow end at 875 MHz

  VCO      = REF * DIV_RATIO   = 875 MHz
  pll_clk  = VCO / 2           = 437.5 MHz   <- the line rate
  test_clk = VCO / 4           = 218.75 MHz   <- on analog_pin[1]

  DIV_RATIO = 4.375, code 35 = 0000100011
              integer part 4 in div_ratio[9:3], 3/8 in div_ratio[2:0]
...
f_pll is averaged over ~525 cycles late in the run rather than across two
...
descr="Load waves"
tclcommand="xschem raw_read $netlist_dir/sg13cmos5l_chipalooza_analog_project_tb_pll_ref200_r4p375.raw tran"
}
...
* Second: is pll_clk on target?  Averaged over 525 cycles, because the
* N/N+1 divider makes any single period the wrong thing to measure.
meas tran t1 WHEN v(x1.pll_clk)=0.6 RISE=1968
meas tran t2 WHEN v(x1.pll_clk)=0.6 RISE=2493
let f_pll = 525/(t2-t1)
let f_pll_target = 4.375e+08
