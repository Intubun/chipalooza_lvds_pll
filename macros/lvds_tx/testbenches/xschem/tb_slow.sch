...
S {}
E {}
T {tb_slow  --  the other end of the rate range: 500 Mb/s

The rate README.md actually asks for.  Same PRBS7, twice the bit period.

This is the comfortable operating point.  1 Gb/s is the top of what the PLL is
specified to deliver, and the pre-driver runs out between 2 and 3.125 Gb/s.

WHAT TO LOOK AT: the same numbers as tb_prbs.  They should be slightly better,
because a longer bit gives every settling tail more time to finish.
...
C {vsource.sym} -2300 -1300 0 0 {name=V1 value="PULSE(0 1.2 0 10p 10p 990p 2000p)" savecurrent=false}
...
T {bit clock: 2000 ps period = 0.50 Gb/s} -2400 -1420 0 0 0.3 0.3 {}
C {vsource.sym} -2100 -1300 0 0 {name=V2 value="PULSE(0 1.2 4000p 5p 5p 4000p 1u)" savecurrent=false}
...
T {DI12 reset: pulse at 4000..8000 ps, deliberately not at t=0} -2200 -1460 0 0 0.3 0.3 {}
...
logy=0
hilight_wave=-1
rawfile=$netlist_dir/tb_slow.raw}
...
meas tran vodmax MAX vod from=34.0000n to=186.0000n
meas tran vodmin MIN vod from=34.0000n to=186.0000n
meas tran vosmax MAX v(vos_d) from=34.0000n to=186.0000n
meas tran vosmin MIN v(vos_d) from=34.0000n to=186.0000n
...
descr="load waves"
tclcommand="xschem raw_read $netlist_dir/tb_slow.raw tran"
}
