...
logy=0
hilight_wave=-1
rawfile=$netlist_dir/tb_alt.raw}
T {tb_alt  --  clause 4.1.5 exactly as the standard writes it

The alternating pattern, which is what 4.1.5 prescribes for dynamic output
signal balance: Vos peak-to-peak has to stay inside 150 mV.

Every bit here is a transition bit, so the driver's auxiliary stage boosts on
every single one and the de-emphasised level never appears.  That is the point
of the clause and also its blind spot -- it hides both the second output level
and all the ISI.  tb_prbs.sch is where those show up.

WHAT TO LOOK AT: vospp in the log, against 150 mV.
...
bench instead:
  docker exec iic-osic-tools_xserver bash -lc \\
    /foss/designs/chipalooza_lvds_pll/macros/lvds_tx/serdes/run.sh} -3510 -2630 0 0 0.4 0.4 {}
...
T {DI9 DI8 = 10  alternating 1010} -1900 -1420 0 0 0.3 0.3 {}
...
C {vsource.sym} -1700 -1300 0 0 {name=V4 value="1.2" savecurrent=false}
...
meas tran vodmax MAX vod from=17.0000n to=53.0000n
meas tran vodmin MIN vod from=17.0000n to=53.0000n
meas tran vosmax MAX v(vos_d) from=17.0000n to=53.0000n
meas tran vosmin MIN v(vos_d) from=17.0000n to=53.0000n
...
descr="load waves"
tclcommand="xschem raw_read $netlist_dir/tb_alt.raw tran"
}
