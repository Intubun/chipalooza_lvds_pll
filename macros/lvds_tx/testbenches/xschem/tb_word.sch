...
S {}
E {}
T {tb_word  --  does the serializer serialize, and is the pre-emphasis there?

The parallel word 0xB4 = 1011 0100 at 1 Gb/s, repeating.  It is chosen for what
it contains: a 11, a 00 and an alternating run, so both output levels appear
inside one byte.

WHAT TO LOOK AT:

 1. Clk / DO0 / D_p in the first graph.  DO0 is the byte strobe, high for the
    bit carrying DI7, so the byte reads off the wire directly and must be
    1 0 1 1 0 1 0 0, MSB first.  A flat line means the .so is missing or stale.
 2. The log.  b0..b7 are Vod at the centre of the eight bits: the SIGN is the
    bit, the MAGNITUDE is which pre-emphasis level it got.  A bit right after a
    transition is boosted to about 430 mV; the second bit of a run falls back to
    about 310 mV.  That gap is the 2 dB of pre-emphasis working on real data,
    through an analog tap that is nowhere near exact.
 3. Vod against the 250..450 mV window of clause 4.1.1.
...
logy=0
hilight_wave=-1
rawfile=$netlist_dir/tb_word.raw}
...
meas tran b0 FIND vod AT=13.5000n
meas tran b1 FIND vod AT=14.5000n
meas tran b2 FIND vod AT=15.5000n
meas tran b3 FIND vod AT=16.5000n
meas tran b4 FIND vod AT=17.5000n
meas tran b5 FIND vod AT=18.5000n
meas tran b6 FIND vod AT=19.5000n
meas tran b7 FIND vod AT=20.5000n
write tb_word.raw
.endc
"
}
...
descr="load waves"
tclcommand="xschem raw_read $netlist_dir/tb_word.raw tran"
}
