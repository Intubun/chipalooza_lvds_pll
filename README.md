...
> [!WARNING]
> **Only the netlist side of this repository has been migrated to the real Chipalooza
> interface.** `schematic/`, `testbenches/` and `submission.yaml` now describe
...
> `floorplan/*.gds`, `layout/*.gds`, `final/` and everything DRC/LVS/PEX still carry the
> HeiChips-style frame (`clk`/`ena`/`rst_n`/`ui_in`/`uio_*`/`uo_out`/`analog_0…2`) that this
> repository was seeded from. The slot geometry has to come from the harness repository
> before any of the physical targets mean anything.

## Project interface

The top cell is generated from `scripts/gen_top.py` and carries the wrapper's port list:

```
enable                 project enable
clk                    shared external clock  (unused - the reference has its own pad)
dig_in[23:0]           24 shared digital inputs
dig_out[11:0]          12 shared digital outputs  (not connected yet)
ref_clk, pll_out, d_p, d_n     the four DEDICATED analog pads
ibias[1:0]             shared current biases
vbias                  shared voltage bias
analog_bus[3:0]        shared analog buses  (not connected yet)
vdd_3v3, vdd_1v2, vss_3v3, vss_1v2, vssio
```

Four dedicated pads means wrapper `_4a`, and therefore **slot `s1` or `s16`** — per
`config.txt` in the harness repository those are the only two with four. All four are
`sg13cmos5l_IOPadAnalog`, so each pad is one core signal carrying the pad's own name;
choosing a different pad type changes the core-facing signals (an `InOut` pad becomes
five: `_in`, `_out`, `_ena`, `_one`, `_zero`).

| pad | signal | |
|---|---|---|
| `analog_pin[0]` | `ref_clk` | PLL reference in |
| `analog_pin[1]` | `pll_out` | PLL clock out — **reserved**, nothing drives it until the PLL is placed |
| `analog_pin[2]` | `d_p` | LVDS out + |
| `analog_pin[3]` | `d_n` | LVDS out − |

Bias comes straight off the harness — `ibias[0]`/`ibias[1]` are the transmitter's two
30 µA references and `vbias` its 1.25 V common-mode reference — so the provisional
bias ports the earlier frame needed are gone, and no on-chip bias block is required.

`dig_in` map. The housekeeping SPI routes every bit individually to a pin, a constant or
the sequencer, so a configuration bit costs a register write and no pin:
...
All-zero on the harness with the project unselected leaves the clock stopped and the
output pair static, which is the idle state the harness requires.

The top-level bench drives PRBS-7 at 1 Gb/s through the pre-driver and the driver into
49.9 + 49.9 Ω and measures |`Vod`| = 358 mV and `Vos` = 1.244 V — both inside
TIA/EIA-644-A.
